...
public class SunFlower implements Flower{
    ...
    private String name;
    public SunFlower(int amount, double price, String name){
        this.amount=amount;
        this.price=price;
        this.name=name;
    }
    public double accept(FlowerVisitor flowerVisitor, int amount, double userMoney, boolean isOwner){
        if(!isOwner){
            if(amount>this.amount){
                System.out.println("We do not have that many Sunflowers in stock!");
                return 0;
            }
            ...
            if(actualAmount>0){
                System.out.print("You bought "+actualAmount+" Sunflowers for $");
                System.out.printf("%.2f",actualPrice);
                System.out.println();
                System.out.println("There are now "+getAmount()+" Sunflowers in stock.");
            }
            return actualPrice;
        }
        else{
            ...
            if(userMoney<orderPrice){
                ...
                this.amount+=amount;
                System.out.print("You have bought "+amount+" Sunflowers for $");
    ...
    public int checkInventory(int amount){
        ...
        if(this.amount==0){
            System.out.println("Sunflowers are out of stock!");
        }
        return actualBought;
    }
}
